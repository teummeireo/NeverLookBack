...
<script>
    ...
    function renderExam(examData) {
        ...
        for (var i = 0; i < examData.questions.length; i++) {
            ...
            var answerObj = answers.find(function (a) {
    ...
    document.getElementById("submit-exam").addEventListener("click", function () {
        ...
        this.disabled = true;
        var url = "${pageContext.request.contextPath}/api/exams/results/" + resultId + "/status?isReviewed=true";
        fetch(url, {
            method: "PUT",
        })
            .then(response => response.json())
            .then(data => {
                if (data.code === 200) {
                    alert("검토를 완료했습니다.");
                    location.href = "${pageContext.request.contextPath}/api/exams/results/" + examId + "/lists";
                } else {
                    alert("채점 저장 요청 실패: " + data.msg);
                }
            })
            .catch(error => console.error("채점 저장 요청 실패:", error));
