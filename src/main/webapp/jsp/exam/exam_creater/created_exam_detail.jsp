...
<script>
    ...
    function markAdminView() {
        ...
        for (var i = 0; i < numberOfQuestions; i++) {
            ...
            userAnswer.textContent = "응시자 답변: " + answers[i];
            questionDiv.appendChild(userAnswer);

            var correctAnswer = document.createElement("p");
            correctAnswer.textContent = "정답: " + questions[i].correctAnswer;
            correctAnswer.style.color = "red";
            questionDiv.appendChild(correctAnswer);

            // ✅ 이의제기한 경우 해당 내용을 추가 표시
            if (answers[i].isObjection) {
                ...
                if (answers[i].objectionReply) {
                    ...
                    replyComment.className = "admin-reply";
                    replyComment.textContent = "관리자 답변: " + answers[i].objectionReply;
                    disputeSection.appendChild(replyComment);
                }

                var replyButton = document.createElement("button");
                replyButton.textContent = "이의제기 답변 달기";
